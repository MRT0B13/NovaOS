...
class PostgresSecretsStore implements SecretsStoreWithClose {
  ...
  static async create(databaseUrl: string): Promise<PostgresSecretsStore> {
    const pool = new Pool({ connectionString: databaseUrl, max: 2, idleTimeoutMillis: 30_000, connectionTimeoutMillis: 10_000, keepAlive: true, keepAliveInitialDelayMillis: 10_000 });
