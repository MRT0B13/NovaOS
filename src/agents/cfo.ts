      ...
      try {
        switch (d.type) {
          ...
          case 'HL_PERP_NEWS': {
            ...
            logger.info(`[CFO] Persisted ${d.type}: ${side} ${coin} $${Number(sizeUsd).toFixed(2)} @ ${leverage}x (conviction=${Number(p.conviction).toFixed(2)})`);
            break;
          }
          ...
          case 'KAMINO_BORROW_LP': {
            ...
            logger.info(`[CFO] Persisted KAMINO_BORROW_LP [${borrowLpTier}]: borrowed $${borrowUsd.toFixed(0)} → ${borrowLpPair} LP`);
